...
- **Multi-column layouts collapse to one column** on small screens. The breakpoints this site uses: the Writings featured card stacks at ≤1150px; article sidebars hide at ≤1024px; content grids collapse at ≤900–960px; phone-level adjustments (nav, footer, padding, type) at ≤640px. New layouts must fit this scheme. The featured card stacks earliest on purpose: its cover is a fixed 1200/630 image beside a text column whose height is set by the title and summary, so the narrower the columns, the more leftover height appears under the cover.
