...
codex exec "Respond with exactly the word: pong"</code></pre>
  ...
  <p><a href="https://models.bytefuture.ai/signup">models.bytefuture.ai</a> で登録し（カード不要、初回チャージで最大 50 ドルの 100% マッチボーナス）、キーを <code>TOKEN_STATION_API_KEY</code> に入れて <code>pong</code> チェックを実行しよう。キー 1 つ、エンドポイント 1 つで、Codex セッションに必要なすべてのモデルが手に入る。</p>
